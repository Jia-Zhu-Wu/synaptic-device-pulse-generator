...
class WaveGeneratorGUI(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(cycle_frame, text="--- 新增脈衝週期 ---").grid(row=0, column=0, columnspan=6, pady=(2, 2))
        ctk.CTkLabel(cycle_frame, text="脈衝振幅").grid(row=1, column=0, padx=2)
        self.wave_number_entry = ctk.CTkEntry(cycle_frame, placeholder_text="整數", width=70)
        ...
        ctk.CTkLabel(cycle_frame, text="間隔零").grid(row=1, column=4, padx=2)
        self.interval_entry = ctk.CTkEntry(cycle_frame, placeholder_text=">=0", width=60)
        ...
        self.add_cycle_btn.grid(row=2, column=2, columnspan=4, padx=2, pady=(2, 1), sticky="w")

        self.cycle_list_label = ctk.CTkLabel(cycle_frame, text="目前已加入脈衝週期：0")
        self.cycle_list_label.grid(row=3, column=0, columnspan=6, sticky="w", pady=(2,0))
        self.cycle_history_box = ctk.CTkTextbox(cycle_frame, width=480, height=80, font=("Microsoft JhengHei", 12))
        self.cycle_history_box.grid(row=4, column=0, columnspan=6, padx=4, pady=(2,2))
        ...
        ctk.CTkLabel(end_frame, text="結尾零數量（預設0）:").grid(row=0, column=0, padx=4)
        self.ending_zeros_entry = ctk.CTkEntry(end_frame, placeholder_text="0", width=90)
        self.ending_zeros_entry.grid(row=0, column=1, padx=2)
        ctk.CTkLabel(end_frame, text="輸出檔名（預設output.csv）:").grid(row=0, column=2, padx=4)
        self.output_file_entry = ctk.CTkEntry(end_frame, placeholder_text="output.csv", width=120)
        self.output_file_entry.grid(row=0, column=3, padx=2)

        self.generate_btn = ctk.CTkButton(end_frame, text="產生CSV", width=100, command=self.generate_csv)
        self.generate_btn.grid(row=1, column=0, columnspan=4, pady=(10, 2))

        self.result_label = ctk.CTkLabel(end_frame, text="")
        self.result_label.grid(row=2, column=0, columnspan=4, pady=(2, 2))
    ...
    def add_cycle(self):
        try:
            ...
            if not (duration > 0 and interval >= 0 and repeat > 0):
                raise ValueError
        except:
    ...
    def generate_csv(self):
        try:
            opening_number = int(self.opening_number_entry.get()) if self.opening_number_entry.get() else 0
        except:
